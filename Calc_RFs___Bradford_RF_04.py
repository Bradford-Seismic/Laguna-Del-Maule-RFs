...
snr_BB = snr[snr.Filtered_SNR > 1.5][(snr.Network != 'XN') | (snr.Network != '1X')].reset_index(drop = True).copy()
...
os.chdir(active_dir)
snr_pass = pd.read_csv('../DATA/SPREADSHEETS/SNR_Summary_Passing.csv')
snr_pass['Station'] = snr_pass['Station'].astype(str)
...
def Calc_RF(path, Z_file, R_file, T_file, net, stat, R_comp, event, phase):
    ...
    Gaussians=(02.5 05.0 07.5)    #Array of Gaussian values
                 """
...
for i in range(len(snr_pass)):
    ...
    t_shift = moveouts.T_Shift[moveouts.Event == event]
    ...
    try:
        response = read_inventory(os.path.join('../DATA/', 'response_xml', '{}.{}.response.xml'.format(net, stat)))
    except:
    ...
    for f in files:
        ...
        tr.trim(starttime = (arrive+t_shift ) - 10, endtime = (arrive+t_shift) + 50)
