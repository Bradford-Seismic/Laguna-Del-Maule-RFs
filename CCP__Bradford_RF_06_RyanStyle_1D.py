...
itd_pass = itd_pass[itd_pass.Accept == 'Man QC Pass'][itd_pass.G == G][itd_pass.Region == region][itd_pass.Phase == 'P']#[itd_pass.Net != '1X']
...
save_region = False     # save the region, specify region name in script

dzi = 0.5   # depth increment in km
z_max = 60 # max depth in km

bin_sz = 3 # node spacing in km
bin_sm = 12 # base radius of bin in km around node, will increase to find more pierce points if allowed
bin_gauss = 4
hits_per_bin = 1
...
if select_region:
    ...
    ax.set_xlim([stat_table.stlo.min() - edge_buff, stat_table.stlo.max() + edge_buff])
    ax.set_ylim([stat_table.stla.min() - edge_buff, stat_table.stla.max() + edge_buff])
    ax.set_aspect('equal')
...
if save_region:
    ...
    np.savetxt('../DATA/MAPPING/{}_Foreland_CCP_GridArea.txt'.format(region), area, delimiter = ',')
...
z = np.arange(np.round((-np.max(stzs)) * 1/dzi) * dzi, z_max + dzi, dzi)
...
with fig.subplot(nrows = 1, ncols = 1, figsize=('15c', '15c')):
    with fig.set_panel(panel = 0):
        ...
        with pygmt.config(FONT="14p"):
            ...
            fig.plot(x = 0, y = 0, style = 'c{}c'.format(bin_sm*2*(15/280)), fill = 'yellow', pen = '1p,black', transparency = 30)
            fig.plot(x = 0, y = 0, style = 'c{}c'.format((bin_sm/3)*2*(15/280)), fill = 'darkorange', transparency = 30)
...
for tr in st:
    ...
    within = line_tree.query_ball_point(pierce_coords, bin_sm, return_length = True)
...
def Gaussian_Distance_Weighted(depth):
    ...
    return depth, seis_avg.reshape(gridx.shape), seis_bstd.reshape(gridx.shape), count.reshape(gridx.shape), count_std.reshape(gridx.shape), radius.reshape(gridx.shape), baz_conc.reshape(gridx.shape), baz_std.reshape(gridx.shape)
...
with concurrent.futures.ProcessPoolExecutor(max_workers = 24) as executor:
    ...
    for layer in layers:
        ...
        BAZ_CONC[:,:,np.where(z==layer[0])[0][0]] = layer[6]
        BAZ_STD[:,:,np.where(z==layer[0])[0][0]] = layer[7]
