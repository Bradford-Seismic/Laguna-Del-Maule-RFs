# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 22:57:56 2026
...
model = xr.open_dataset('../DATA/MAPPING/nc_models/' + model_name, engine="netcdf4")
...
gps = pd.read_csv('../DATA/MAPPING/'+gps, sep = '\s+', names = ['time', 'N', 'E', 'D'])


# Fit linear trend
m, b = np.polyfit(gps.time[(gps.time>2012) & (gps.time<2020)], gps.D[(gps.time>2012) & (gps.time<2020)], 1)
...
coeffs = np.polyfit(gps.time[(gps.time>2012) & (gps.time<2020)], gps.D_detrend[(gps.time>2012) & (gps.time<2020)], 3)
...
# Evaluate breakpoints in data using Piecewise Regression
# ms = piecewise_regression.ModelSelection(gps.time.values, gps.D.values, max_breakpoints=8)
print('Calculating Piecewise Regression - piecewise_regression')
num_breakpoints = 6
pw_fit = piecewise_regression.Fit(xx = gps.time.values, yy = gps.D.values, n_breakpoints=num_breakpoints)
...
with pygmt.config(FONT="12p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p", MAP_GRID_PEN="0.01p", MAP_FRAME_PEN="0.25p"):
    ...
    fig.basemap(region=[2012, 2020, -0.1, 0.12], projection=prj, frame=[
    ...
    fig.plot(x = [2018.348, 2018.348], y = [-10, 10], pen = '2p,darkgreen,--')
    fig.plot(x = [2018.125, 2018.125], y = [-10, 10], pen = '2p,red,--')

    # long term gps trends (Le Mevel et al., 2021)
    fig.plot(x = [2010.75 , 2010.75], y = [-10, 10], pen = '3p,lighred,--')
    fig.plot(x = [2016.73 , 2016.73], y = [-10, 10], pen = '3p,lightred,--')
    
    
    # plot results from piecewise-regression
    for change in range(num_breakpoints):     
        break_time = pw_fit.get_results()['estimates'][f'breakpoint{change+1}']['estimate']
        fig.plot(x = np.ones(2) *  break_time,
                 y = [-10, 10], pen = '0.5p,blue,--')
       
        c1 = pw_fit.get_results()['estimates'][f'breakpoint{change+1}']['confidence_interval'][0]
        c2 = pw_fit.get_results()['estimates'][f'breakpoint{change+1}']['confidence_interval'][1]

        fig.plot(x = [c1, c2, c2, c1], y = [-10, -10, 10, 10], fill = 'blue', transparency = 90)
...
with pygmt.config(FONT="12p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p", MAP_GRID_PEN="0.01p", MAP_FRAME_PEN="0.25p"):
    ...
    fig.basemap(region=[2012, 2020, 0, 1.9], projection=prj, frame=[
    ...
    # long term gps trends (Le Mevel et al., 2021)
    fig.plot(x = [2010.75 , 2010.75], y = [-10, 10], pen = '3p,lightred,--')
    fig.plot(x = [2016.73 , 2016.73], y = [-10, 10], pen = '3p,lightred,--')
...
all_dat["utc"] = all_dat["O_Time"].apply(utc)
...
all_dat["decimal_year"] = all_dat["utc"].apply(to_decimal_year)
...
S 0.1c r 0.6/0.25 white 0.25p,black 0.65c All Events Histogram
...
S 0.20c i 0.4c lightred 0.6p,black 0.65c LaMa Broadband
...
with pygmt.config(FONT_ANNOT_PRIMARY="12p"):
    fig.legend(spec=spec,  position='jBL+w14c+o{}c/-3.5c'.format(-3.5))


# Plot topography
fig = plot_topo(fig, joints, fig_x, fig_y)

fig.savefig('../FIGURES/Figure6-1.png', dpi = 600)
fig.show()
...
fig.show()
fig.savefig('../FIGURES/Figure6-2.png', dpi = 600, transparent = True)
