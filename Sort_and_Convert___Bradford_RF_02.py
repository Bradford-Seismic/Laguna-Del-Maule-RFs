...
We will now move these files according to where we want to process them
...
print('Num Files downloaded: {}, P: {}, p: {}'.format(len(downloads), P, p))
...
if 'Code' not in downloads.keys():
    codes = ['{}-{}'.format(downloads.Network[i], downloads.Station[i]) for i in range(len(downloads))]

    downloads['Code'] = codes


unique_stations = np.unique(downloads.Code)
print('{} unique stations succesfully downloaded'.format(len(unique_stations)))


# first thing to handle, let's collect all of our station information
stlas = stlos = nets = stats = np.array([])

for i in unique_stations:
        station_df = downloads[downloads.Code == i].copy().reset_index(drop = True)
        stla = station_df.STLA.unique()[0]
        stlo = station_df.STLO.unique()[0]

        stlas = np.append(stlas, stla)
        stlos = np.append(stlos, stlo)

        nets = np.append(nets, i.split('-')[0])
        stats = np.append(stats, i.split('-')[1])
...
# now plot our data
fig.plot(x = stlos, y = stlas, style='i0.2c', fill = 'yellow', pen='black', label = 'Downloaded Station')
...
def make_sac(st, record, destination):
    ...
    inv = read_inventory(os.path.join('../DATA/response_xml', '{}.{}.response.xml'.format(net, stat)))
...
def convert_and_transfer_SAC(contributing_stations, destination):
    ...
    for i in contributing_stations:
        ...
        files = glob(os.path.join('../DATA/raw_mseed/', '{}.{}.*.mseed'.format(net, stat)))
        for j in files:
...
for loc in locations:
    ...
    contributing_stations = unique_stations
