...
def plot_topo(fig, joints, fig_x, fig_y, Numerals='ABCDEFG', no_y = False):

    joints_dist, line, line_dist = Return_Line_Elements(joints)

    fig.shift_origin(yshift='+h')
    with pygmt.config(FONT='14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p/28p"):
        if no_y:
            fig.basemap(region = [0, joints_dist[-1], 0,5], projection = 'X{}c/{}c'.format(fig_x, fig_topo_y), frame = ['wsNe', 'xa20f10+lProfile Distance (km)', 'ya4f1+e+lElev. (km)'])
        else:
            fig.basemap(region = [0, joints_dist[-1], 0,5], projection = 'X{}c/{}c'.format(fig_x, fig_topo_y), frame = ['WsNe', 'xa20f10+lProfile Distance (km)', 'ya4f1+e+lElev. (km)'])
...
with pygmt.config(FONT="14p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    joints = np.loadtxt(
        '../DATA/MAPPING/{}_Line-SN2.txt'.format(region), delimiter=',')
    ...
    # Plot topography
    fig = plot_topo(fig, joints, fig_x, fig_y, Numerals = 'SN', no_y = True)
    ...
    # Plot topography
    fig = plot_topo(fig, joints, fig_x, fig_y, Numerals = 'WE', no_y = True)
...
""".format(eq_range)
    ...
    fig.shift_origin(xshift='{}c'.format(-(fig_x + 0.5)*1), yshift = '{}c'.format(-fig_y))

    # plot legend
    with pygmt.config(FONT_ANNOT_PRIMARY="14p"):
        fig.legend(spec=spec,  position='jBL+w{}c+o1c/-1.7c'.format(fig_x*2))


    fig.shift_origin(xshift='{}c'.format((fig_x + 0.5)*2))
    ...
    fig.basemap(region=[vs.min() - 1, vp.max() + 1, z.min(), z.max()],
                projection='X3c/-{}c'.format(fig_y+0.725), frame=['wSnE', 'xa2f1+lvelocity', 'ya10f5g20+lDepth (km)'])
...
fig.savefig('../FIGURES/F4_CCP_Stack_G-7.5.png', dpi = 1200)
