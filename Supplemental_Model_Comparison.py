...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 17:20:50 2025
...
with pygmt.config(FONT="16p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.text(position = 'BL', text = 'ZR-mod',offset = '0.25c/0.25c', font = '16p,Helvetica,black', pen = '0.5p,black', fill = 'white')
...
model_name = 'CCP_LDM_G-7.5_GDW_VELEST-1X-mod.nc'
model = xr.open_dataset('../DATA/MAPPING/nc_models/' + model_name)
...
with pygmt.config(FONT="16p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.text(position = 'BL', text = 'TANGO-mod',offset = '0.25c/0.25c', font = '16p,Helvetica,black', pen = '0.5p,black', fill = 'white')
...
model_name = 'CCP_LDM_G-7.5_GDW_ANT.nc'
model = xr.open_dataset('../DATA/MAPPING/nc_models/' + model_name)
...
with pygmt.config(FONT="16p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.text(position = 'BL', text = 'ANT',offset = '0.25c/0.25c', font = '16p,Helvetica,black', pen = '0.5p,black', fill = 'white')
...
model_name = 'CCP_LDM_G-7.5_GDW_FWT.nc'
model = xr.open_dataset('../DATA/MAPPING/nc_models/' + model_name)
...
with pygmt.config(FONT="16p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.colorbar(cmap = True, position = 'JMR+w{}/0.4c+o0.25c/0c'.format(fig_y), frame = 'xa0.5f0.1+lAmplitude')

    # overlying grid
    fig.basemap(region=grid_region, projection=prj, frame=[
                'wsne', 'xaf100g20', 'ya20f10g20'])

    fig.text(position = 'BL', text = 'FWT',offset = '0.25c/0.25c', font = '16p,Helvetica,black', pen = '0.5p,black', fill = 'white')
...
fig.shift_origin(xshift='-{}c'.format((fig_x + 0.5)*4), yshift = '{}c'.format(-fig_y))

v1 = 'velmodel_zr-mod.csv'
v2 = 'velmodel_tango-mod.csv'

v3 = 'Andes.ANT.Ward.2013_kmps.nc'
v4 = 'FWT-SouthAmerica-2022.r0.0.nc'



csv_models = [v1, v2]
csv_model_names = ['VELEST-ZR-mod', 'VELEST-TANGO-mod', ]
csv_colors = ['blue', 'red', 'darkgreen']

nc_models = [v3, v4]
nc_model_names = ['ANT', 'FWT']
nc_colors = ['darkgreen', 'purple']
...
def interp_vel(v_name):
    v = pd.read_csv('../DATA/MAPPING/nc_models/'+v_name)

    vs_interp_func = interp1d(v.Depth, v.Vs, kind = 'nearest', bounds_error = False, fill_value = 'extrapolate')
    vp_interp_func = interp1d(v.Depth, v.Vp, kind = 'nearest', bounds_error = False, fill_value = 'extrapolate')

    vs = vs_interp_func(z.values + 0.5*dzi)
    vp = vp_interp_func(z + 0.5*dzi)

    v_pd = pd.DataFrame({'Depth': z.values, 'Vp': vp, 'Vs': vs})

    return  v_pd



# pull 1d profiles from nc models


joints = np.loadtxt('../DATA/MAPPING/{}_Line2.txt'.format(region), delimiter = ',')
joints_dist, line, line_dist  = Return_Line_Elements(joints)
...
def interp_nc(v_name):

    vs = xr.open_dataset('../DATA/MAPPING/nc_models/' + v_name)

    vs_slice = vs.interp(longitude = x, latitude = y, method = 'nearest', kwargs={"fill_value": None})
    vs_slice = vs_slice.assign_coords({'Distance_Along_Trend': line_dist})
    sz = vs_slice.depth.values
    vsu = vs_slice.interp(Distance_Along_Trend = 35, method = 'linear').vs.values

    # remove any prior nan's
    vsu_nan = np.isnan(vsu)
    vsu = vsu[~vsu_nan]
    sz = sz[~vsu_nan]


    minv = 1.8  # lowest allowable shear wave velocity in model, any point lower will be raised to this
    vsu = np.clip(vsu, minv, np.nanmax(vs.vs.values))

    vs_interp_func = interp1d(sz, vsu, kind = 'linear', bounds_error = False, fill_value = 'extrapolate')
    vs = vs_interp_func(z.values + 0.5*dzi)

    vs = np.clip(vs, minv, np.nanmax(vs))


    vpvs = np.ones(vs.shape)
    vpvs[z.values<=50] = 1.74
    vpvs[z.values>50] = 1.79


    v_pd = pd.DataFrame({'Depth': z.values, 'Vp': vs * vpvs, 'Vs': vs})
...
with pygmt.config(FONT="16p", MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    fig.basemap(region=[1, 5, z.values.min(), z.values.max()], projection='X{}c/-{}c'.format(fig_x, fig_y+0.55),
                frame =[ 'WSne', 'xa1f0.5g2+lvs (km/s).', 'ya10f5g20+lz (km)'])

    for i, vel in enumerate(csv_models):
        fig.plot(x = interp_vel(vel).Vs, y = interp_vel(vel).Depth, pen = '1p,'+csv_colors[i], label = csv_model_names[i])

    for i, vel in enumerate(nc_models):
        fig.plot(x = interp_nc(vel).Vs, y = interp_nc(vel).Depth, pen = '1p,'+nc_colors[i], label = nc_model_names[i])

with pygmt.config(FONT_ANNOT_PRIMARY="14p"):
    fig.legend(position='JML+w3.5c+o-3.75c/-4c', box='+gwhite')


fig.show()


fig.savefig('../FIGURES/CCP_Model_Comparison.png', dpi = 300)
